...
export function updateWildMotion(entity, dt, data, playerX, playerY) {
  ...
  groupBehavior.tickGroupCombatBreakLeaderCalm(entity, entitiesByKey, dt);
  ...
  const threatX = playerX;
  const threatY = playerY;
  const dxThreat = dxP;
  const dyThreat = dyP;
  const distThreat = distP;
  const stopDist = beh.stopDist ?? 1.2;
  const groupCombatBreakActive = !!entity.groupCombatBreakActive;
  const groupFollowEarly = groupBehavior.resolveGroupFollowTarget(entity, entitiesByKey);
  const followerTeamMode = !groupCombatBreakActive && !!groupFollowEarly && !groupFollowEarly.isLeader;
  ...
  if (!groupCombatBreakActive) enforceFollowerLeaderMaxDistance(entity, data);

  if (!groupCombatBreakActive && !followerTeamMode && !entity.wildGrassHostileDeathBattle) {
