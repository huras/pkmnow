...
const THUNDER_STRIKE_MP3_URL = new URL('../../audio/sfx/thunder-sfx.mp3', import.meta.url).href;

const POOL_SIZE = 6;
const THUNDER_STRIKE_VOL = 0.78;
...
function ensurePool() {
  if (!pool) {
    ...
    for (let i = 0; i < POOL_SIZE; i++) {
      const a = new Audio(THUNDER_STRIKE_MP3_URL);
      a.preload = 'auto';
      pool.push(a);
    }
  }
...
/**
 * Play thunder strike SFX from the strike world position.
 * @param {{ x?: number, y?: number, visualX?: number, visualY?: number, z?: number } | null | undefined} source
 */
export function playThunderStrikeSfx(source) {
  const a = borrowAudio();
  try {
    a.currentTime = 0;
  } catch {
